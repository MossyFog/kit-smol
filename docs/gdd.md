# Title
## Game Name:
KIT SMOL
or: Kit Smol, kit-smol
## Company/Solo Name
[Game Programming & Development Club](https://gpdmcc.org)
[MossyFog](https://gitlab.com/mossyclouds/mossyfog):
	(MossyCloud's MossyFog subgroup. (F)ree (O)pen (G)ames.)
[MistMoss](https://gitlab.com/mossyclouds/mossyfog/mistmoss):
	...the subgroup of MossyFog, is an experimental - and potentially temporary
	- get-together of loosely-interested individuals who would like to try
	making MD0 licensed content.

## Your Name
Nohbdy Ahtall (NohbdyAhtall)
## Writer's Name
Nohbdy Ahtall
## Any Important Licenses
CC0, MIT

# Overview
## High Concept (Five Sentences or Less)
Uhhh... smol kitteh, bounces around maybe
## Summary (Up to Five Paragraphs)
Aww gee, errr, jeesh,, ummmm oooho uhhh..
## Genre and Target Audience
Genre: Smol Kitteh
Target Audience: Lovers of Smol Kittehs
## Gameflow (Overview)
So far: bounce around as smol kitteh
## Look and Feel
Pixelly
## External Influences
Tiny Cat CC0 asset by Kirisoft
## Monetization
Probs Donationware

# Story
## Back Story
First there was kitteh...
## Main Story
Then, there was kitteh..
## Side Stories (List one by one)
There was also kittehs..
## Story Progression
Kittehs and then Kittehs

# Settings
## Setting Overview
Has kittehs, SMOL ones
## Game World (Include World Building)
HazMat warning-striped tiles everywhere
## Look and Feel
Meow
## Visual and Aural Style/Themes
Purr-Mrrp
## Cinematics
Have you seen the trailer?

# Characters
## Main Characters
### KIT SMOL
A kitteh
#### Back Story
Exists
#### Personality
Bouncy
#### Appearance
(insert pic of kit smol)
#### Story Connection
Is the main kitteh, playable
## Supporting Characters
TBD, meowhaps more kitetehs - no wait, purrobably

## Antagonists

## Background Characters
\* Each should have Back Story, Personality, Appearance, Story Connection, Relationships, Abilities, Location, Schedules.

---

# Gameplay
## Game Progression (Break into Sections)
First you bounce around all wild and uh, uh
## Objectives (Per Section)
Mew?
## Mission Structure
0.0
## Puzzle Structure
00.00
## Balancing (Describe, can link to Spreadsheets)
Wobbly

# Mechanics
## Physics and Controls
You are physicsy and bounce
## Navigation in World
You go directions of the 2D nature and bounce
## Interactivity
You bounce
## Combat and Challenge Resolution
Uhh probably bounce
## In-Game Economy
Bounciness

# Game Options
##  Main Menu Options
None for meow
##  Graphics and Audio Settings
Nuffin
##  In-Game Options
Some buttons!!
##  Pause Menu
Oooh that's a good idea huh?
##  When Can Change?
Jeesh iono.. hmmmmm

# Saving and Loading
## Loading from Main Menu
Im-paws-able... for now
## Saving or Loading within Game
I mean... one might hope
## Autosaving Quicksaving
Seems a nifty idea!
## Save Management
Completely random save gets loaded - hehe jk
## Cloud Backup
No-no cloud, come closer ish okay

# Replaying
## What makes game replayable?
Starting the game again!
## Player Choice and Outcome
You bounce and then... hm
## Multiple Paths (More Detail)
Well, lots of places to bounce..
## Multiple Endings (More Detail)
You could be on a tile, or fall foreverishly
## New Game+
(paws the plus symbol around)

---

# Cheats
## Developer Cheats
Haxx? Where? O, for me...
## Intentional Cheats
Haxx!!!
## Possible Exploits
(Paws the exploits)

# Easter Eggs
## Easter Egg List
Nom
## How To Find
Wha?

# Tutorial Level
## Synopsis
(blinks)
## Introduction
Hello!
## Physical Description
smol
## Level Flow
squarey
## Critical Path
Openings between sqaurrs
## Challenges and Solutions
Bounce or do not bounce, up to mew

# Game Levels (Repeated)
## Synopsis
World of squares, pre'y smol tho
## Introduction
Hi again.
## Physical Description
Stil smol
## Objectives
Bounce
## Level Flow
Thatta way
## Critical Path
No wait, thattaway
## Optional Paths
Okay actually hang on lemme fink about this...
## Important Challenges and Solutions
I mean, which way, really?
## Trivial Challenges and Solutions
(falls)
## Level Conclusion(s)
You Fell

# User Interfaces
## Control Systems
Keybread
## Cameras
It follows, wif delay
## Menu Systems
There "is" a button, I mean, look at that!
## HUDs and Minimaps
You see those corner interfaces!? Ready for multi-angle lookin!
## Help Systems
Hello Systems, what do you need help wif?
## UI Visuals and Audio
I like those

---


# Game Engine
## Game Engine and Version
Godot 4.5 and beyooond!
## Relevant Version Notes
Uhhh, be pre-purred for the newests
## Highest Engine Upgrade Possible
Maximummm! Well, maybe not Dev & RC builds

# Game Programming
## Programming Languages Used
GDScript
## In-House Plugins and Scripts
Ooh yes pelase
## Third-party Plugins and Scripts
Might not have any :0
## Version List and Upgradability for Everything
Kit Smol: This one
- Upgrade by coding more

# Game AI
- AI Overview and Methods
- Friendly AI
- Neutral AI
- Hostile AI (Out of Combat)
- Hostile AI (In Combat)

# Game Art
- Art Style
- Visual Themes
- Concept Art
- Environments
- Characters
- Objects
- Miscellaneous

# Game Audio
- Aural Theme
- Background Music
- Environmental Audio
- Sound Effects
- Menu Sounds
- Voice Acting

---

# Technical Specs

- Target Hardware
- FPS on Test Systems (Late Beta to Gold)
- External Libraries (Visual Studio, Etc)

# Design Plan
- Team Members and Roles
- Production Phases (POC to Gold)
- Testing Milestones
- Minimum For Release
- Early Access Plan
- Marketing Milestones

# Installation and Update Plan
- Installation Methods
- External Install Software
- Upgrade Methods
- Roll-Back Versions?
- Beta Versions?

# Proposed Schedule
- POC to Alpha
- Alpha to Beta
- Beta to Gold
- Gold to Release
- Post-Release

# Asset List
- Art and Animation List
- Audio and UI Sounds List
- Special Effects List
- Code and Plugin List
- Full VA Scripts (Links are OK!)
